...
const sources = [
  { file: 'furniture.js', target: 'house_furniture.json' },
  { file: 'clothes.js', target: 'clothes.json' },
  { file: 'body.js', target: 'body_health.json' },
  { file: 'food_drink.js', target: 'food_drink.json' },
  { file: 'dishes.js', target: 'food_drink.json' },
  { file: 'animals.js', target: 'animals.json' },
  { file: 'colours.js', target: 'colors.json' }
];
...
sources.forEach(src => {
  ...
  const targetPath = path.join(cdDir, src.target);
  ...
  items.forEach(item => {
    ...
    const themeName = src.target.replace('.json', '');
    const newEntry = {
      id: id,
      word: wordClean,
      language: 'it',
...
console.log(`Batch 2 Complete: Added ${addedNew} new entries, updated ${updatedLevels} levels in higher files, skipped ${skippedDuplicates} existing A0/A1 duplicates.`);
