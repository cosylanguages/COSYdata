...
// Batch 4 sources and target mappings
const sources = [
  { file: 'locations.js', target: 'geography.json' },
  { file: 'places.js', target: 'places_transport.json' },
  { file: 'travel.js', target: 'places_transport.json' },
  { file: 'nationalities.js', target: 'nationalities.json' },
  { file: 'school.js', target: 'school.json' },
  { file: 'jobs.js', target: 'jobs.json' },
  { file: 'people.js', target: 'geography.json' }, // Famous historical figures/countries/people -> geography or common
  { file: 'shopping.js', target: 'places_transport.json' }
];
...
sources.forEach(src => {
  ...
  items.forEach(item => {
    ...
    let targetFile = src.target;
    // Proper country/city names go to geography.json
    if (['Francia', 'Italia', 'Spagna', 'Germania', 'Inghilterra', 'Roma', 'Parigi', 'Londra'].includes(wordClean)) {
      targetFile = 'geography.json';
    }
    ...
    // Def & Ex
    let defText = 'Termine relativo a ' + wordClean + '.';
    let examples = [];
    ...
    if (pos === 'noun') {
      const isProperNoun = /^[A-Z]/.test(wordClean) && ['geography.json', 'nationalities.json'].includes(targetFile);
      if (isProperNoun) {
...
console.log(`Batch 4 Complete: Added ${addedNew} new entries, updated ${updatedLevels} levels in higher files, skipped ${skippedDuplicates} existing A0/A1 duplicates.`);
